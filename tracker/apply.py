...
def get_info(im, detections):
	...
	for d in detections:
		if d['class'] != 'car':
			continue
		d['left'] = clip(d['left']*im.shape[1]//1280, 0, im.shape[1])
		d['right'] = clip(d['right']*im.shape[1]//1280, 0, im.shape[1])
		d['top'] = clip(d['top']*im.shape[0]//720, 0, im.shape[0])
		d['bottom'] = clip(d['bottom']*im.shape[0]//720, 0, im.shape[0])
...
def process_video(fname, json_fname, out_fname):
	...
	vreader = ffmpeg.Ffmpeg(fname, 640, 352)
	for frame_idx, dlist in enumerate(detections):
		...
		while len(new_detections) <= frame_idx:
			new_detections.append([])
		new_detections[frame_idx] = [d for (d, _) in cur_info]
		...
		if prev_info is None or len(prev_info) == 0:
			...
			for idx in range(len(cur_info)):
				...
				new_detections[frame_idx][idx]['track_id'] = track_counter
				track_counter += 1
		...
		for i in range(len(active_objects)):
			for j in range(len(cur_info)+1):
				...
				#	output = -100.0
				if j != len(cur_info) and longim_out_logits[i, j] < 0:
					output = -100.0
		...
		for idx1, obj in enumerate(active_objects):
			if idx1 in forward_matches:
				...
				used_idx2s.add(idx2)
				new_detections[frame_idx][idx2]['track_id'] = obj[0]
			elif obj[3] < MAX_AGE:
				idx2 = votes[idx1][1]
		...
		for idx2 in range(len(cur_info)):
			...
			new_detections[frame_idx][idx2]['track_id'] = track_counter
			track_counter += 1
...
for fname in os.listdir(video_path):
	...
	video_fname = video_path+str(id)+'.mp4'
	json_fname = json_path+str(id)+'.json'
	out_fname = out_path+str(id)+'.json'
	print('process', video_fname)
	process_video(video_fname, json_fname, out_fname)
