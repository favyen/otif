...
video_path = root_path+'tracker/video/'
json_path = root_path+'tracker/'+detector+'/'
match_path = root_path+'tracker/preproc/'
model_path = root_path+'tracker/model2/model'
...
for label in labels:
	print('... {}'.format(label))
	video_fname = video_path+label+'.mp4'
	json_fname = json_path+label+'.json'
	match_fname = match_path+label+'.json'
...
def get_random_info(exclude_label):
	...
	while True:
		...
		if len(frame_infos[frame_idx]) > 2:
...
for label in labels:
	...
	for i, frame_idx in enumerate(frame_infos.keys()):
		...
		neg_info = [el for _ in range(8) for el in get_random_info(label)]
...
train_examples = [example for example in all_examples if example[4] not in val_labels and min(example[2][:-1]) >= 2]
